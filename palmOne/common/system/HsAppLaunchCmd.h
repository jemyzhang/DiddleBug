...
// <chg 2006-06-07 ericw>
// This is used to add a number to an existing contact in PhoneApp. Currently
// the post call number store uses this
//-------------------------------------------------------------------
// Support for addrAppNotificationAddToRecord
//-------------------------------------------------------------------
...
typedef struct
  {
	Char*	data;     /**< Data to save (Null terminated string) */
	UInt32  uniqueID; /**< The UID of the contact to save the phone number to */
	UInt8	field;    /**< Phone number, URL, or email address field number.
                          *  They are defined in ViewForm.c as follows:
                          *  #define kAddrDBRecMobileFld 5   // phone3
                          *  #define kAddrDBRecEMailFld  7   // phone5
...
AddrAddToRecordParamsType, * AddrAddToRecordParamsPtr;
//-------------------------------------------------------------------
...
#define	AddrHsLookupUserDataVersion2		1 /**< Version number of the addrAppNotificationHsLookup command for the V2 structure */
...
typedef struct
  {
	...
	Boolean					useMultiFilter;	/**< IN: TRUE if lookup should use multifilter.
													 FALSE if lookup should NOT use multifilter. */
	Boolean					displaySIMbook;	/**< IN: TRUE if lookup should display Simbook entries.
													 FALSE if lookup should NOT display Simbook entries. */
  }
AddrHsLookupUserDataV2Type, *AddrHsLookupUserDataV2Ptr;
...
// Query the phone application for the caller id for a given number
#define phoneAppLaunchCmdCallerIDLookup (sysAppLaunchCmdCustomBase+25)

// Re-register all the Phone App views
...
typedef enum
  {
	...
	PhoneAppDialMethodSATSetupCall,	  /**< Use SIM Toolkit and respond 
                                            affirmative to the SIM's request 
                                            to set up a call. */ 
        // The below codes are added to allow passing of the call setup 
        // options
...
/** Contains data used by phoneAppLaunchCmdCallerIDLookup */
typedef struct
  {
	Char*   numberP;			/**< the view type to display								           be applied; if zero -- don't apply. */
	Char*	nameP;				/**< if not NULL, the view manager will pass this data block to the view
								     once the view has been opened */
  }
PhoneAppLaunchCmdCallerIDLookupType, PhoneAppLaunchCmdCallerIDLookupPtr;
