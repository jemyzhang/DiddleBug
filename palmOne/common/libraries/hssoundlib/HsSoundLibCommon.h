 ...
 *
 * @brief	Public 68K/Arm common header file for sound support for Treo 600,
 * 			Treo 650, and Treo 700p smartphones and later models.
 *
 ...
 */
/*@{*/
#define hsSndLibVersion				0x010D0000	    /**<Version of the sound library. */
 ...
 *
 * Note: Each one of these notifications are reserved as Creator IDs with PalmSource
 */
/*@{*/
...
#define hsSndNotifyNotHybrid        'hNoH' /**<Currently attached peripheral on headset jack is Not a hybrid headset (see hsSndNotifyHybrid). */
...
#define hsSndNotifyAthenaAudio      'hAta' /**<Automatic audio switches will route to Athena connector from now on (Treo-650 and later). */ 
/*@}*/

/**
 * @name Sound library resource types
 *
 */
/*@{*/
#define hsSndRsrcAudioTableType     'hTbl' /**<Resource contains audio table that can be loaded to audio driver. */
#define hsSndRsrcAudioTableID       25000  /**<Default audio table ID which can be loaded automatically during reset. */
/*@}*/
 ...
 */
/*@{*/
#define hsSndErrBase				hsSoundLibErrorClass /* 0x7500 == 29952 */
...
#define hsSndErr_UnknownClkRef      (HS_SND_ERR(17)) /**<Unknown clock reference passed to synchronization clock utility */
#define hsSndErr_InvalidPrefValue   (HS_SND_ERR(18)) /**<Unsupported or invalid audio preference value */
...
#define hsSndPortID_InBtHeadset		    HS_AC97(0x00000015) /**<Input port used by OutRadioMic switch to get uplink audio from BT headset. */
#define hsSndPortID_InBtHandsfree		HS_AC97(0x00000016) /**<Input port used by OutRadioMic switch to get uplink audio from BT handsfree mode. */
#define hsSndPortID_LastPort			HS_AC97(0x00000016) /**<Update at end. Same ID as last defined port in sequence. */
/*@}*/
...
enum HsSndFormatEnumTag
{
    ...
    pmSndFormatPauseStream, /**<For internal use only (escape code used by PmSndStreamPause macro function). */
    pmSndFormatResumeStream, /**<For internal use only (escape code used by PmSndStreamResume macro function). */
    pmSndFormatSetStoppable /**<For internal use only (escape code used by PmSndStreamStoppable macro function). */
};
...
#define hsSndBufFlags_attackEnvVibeNone   0x0020 /**< Use special attack envelope with initial vibration before envelope */
#define hsSndBufFlags_attackEnvVibeRings  0x0030 /**< Use special attack envelope for ringtones with initial vibration before envelope */
#define hsSndBufFlags_attackEnvRings2     0x0040 /**< Use type 2 attack envelope suitable for call ringtones (stright ramp up) */
#define hsSndBufFlags_attackEnvReserved1  0x0050 /**< Reserved for future use. Do Not use */
#define hsSndBufFlags_attackEnvVibeRings2 0x0060 /**< Use type 2 attack enveloper with initial vibration before envelope */
#define hsSndBufFlags_attackEnvReserved2  0x0070 /**< Reserved for future use. Do Not use */
#define hsSndBufFlags_attackEnvMask       0x00F0 /**< Mask reserves up to 16 different envelope IDs. */

#define hsSndBufFlags_interruptDefault	  0x0000 /**< (Default) interrupt sound on most user events if hsSndBufFlags_canInterrupt flag is set. */
#define hsSndBufFlags_interruptPhone	  0x0100 /**< interrupt on user events (except Touch, Navigation arrows and pens down). */
#define hsSndBufFlags_interruptModeMask	  0x0300 /**< Mask reserves 2 bits for up to 4 different interrupt modes. */
/*@}*/
 ...
 */
/*@{*/
#define hsSndAmplitude_silent			((UInt32) 0)    /**< Minimum volume value used to silent a port */
#define hsSndAmplitude_unity			((UInt32) 1024) /**< Volume value used to pass through sound without amplification or attenuation. */
/*@}*/
...
#define pmSndAttackEnvIDVibeRing    (hsSndBufFlags_attackEnvVibeRings)  /**< Vibrate for 4 seconds, then behave as pmSndAttackEnvIDRings */
#define pmSndAttackEnvIDRings2      (hsSndBufFlags_attackEnvRings2)     /**< The ringtone attack envelope ramp-up in about 1 second */
#define pmSndAttackEnvIDVibeRing2   (hsSndBufFlags_attackEnvVibeRings2) /**< Vibrate for 4 seconds, then behave as pmSndAttackEnvIDRings2 */
...
#define pmSndPrefID_AthenaActive		PM_AUDP_CLASS(0x0007) /**< set to activate audio paths to multi-connector, clear to return to normal (used for wired carkit mode) */
...
#define pmSndPrefID_AthenaAudio         PM_AUDP_CLASS(0x000A) /**< set to activate or deactivate automatic routing of audio to Athena connector */
...
// Parameters to use with pmSndPrefID_AthenaAudio
#define pmSndPref_AthenaAudio_NoRouting     (0)  /**< no athena audio routing when Athena audio connection detected. Handled elsewhere or through pmSndPrefID_AthenaActive */
#define pmSndPref_AthenaAudio_IfMicPresent  (16) /**< when Athena audio class detected, only route if Mic is also present */
#define pmSndPref_AthenaAudio_Unconditional (32) /**< cause unconditional audio routing if Athena audio class detected */
...
// ===============================================================================================
// Media Synchronization Definitions
// ===============================================================================================
typedef UInt32   PmClkTickRef;

#define pmAudioClkTickRef   ((PmClkTickRef) 0x00000001) /**< use as PmClkTickRef when requiring audio tick clock */
