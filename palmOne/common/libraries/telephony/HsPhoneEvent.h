...
typedef enum
  ...
  *************************************************************/
  ...
  /** Data session status change
   *  This should replace the OneXStatus, as this will encompass more info
   *  than OneXStatus
   */
  phnEvtDataSessionStatus,           /**< 0x002C   */

  /** Data session disconnected
   *  This is sent up when the data call is disconnected.
   *  We can't use the phnEvtDisconnectInd because the structure uses the
   *  Err class.
   */
  phnEvtDataSessionDisconnected,     /**< 0x002D   */

  /** Returns the network time and DST obtained from network to date time panel
   */
  
  phnEvtNetworkTime, /**< 0x002E   */
...
typedef struct {
    UInt32  signature;      /**<        */
    UInt32  notificationType;   /**<        */
    UInt32  EFList;       /**< This is a bitmap of the efs to be refreshed*/
    UInt16  refreshType;  /**<        */
    UInt16  pad;
  } PhnSATEventRefresh;
...
// for notificationType = kSATIdleModeText
/**
 *
 **/
typedef struct {
    ...
    UInt32  alphaIdOffset;
} PhnSATEventIdleModeText;

// for notificationType = kSATIdleRunAT
/**
 *
 **/
typedef struct {
...
// for notificationType = kSATDataSvc
/**
 *
 **/
typedef struct {
  ...
  UInt8   chanId;
  UInt8   serviceType;      /** See GSMSATDataSvcType */
  UInt8 loginNeeded;
  UInt8 passwdNeeded;
  UInt32  alphaIdOffset;
} PhnSATEventDataSvc;

// for notificationType = kSATSendDTMF
/**
 *
 **/
typedef struct {
  ...
  union Data
    {
      ...
      PhnDataChannelType       dataChannelAvailability; /**< phnEvtDataChannel event <currently not used> */
      PhnOnexMipRRQType        PhnOnxMipRRQ;            /**<        */
      PhnOneXDataFailType      oneXFailReason;          /**<        */
      WAPNotificationEventType wapEvtParams;            /**<        */
      PhnDataSessionStateType  dataSesstionStatus;      /**< Sent when the data session state changes */
      PhnNetworkTime           networkTime;  /**< GSM Only, phnEvtNetworkTime */
