 ...
 *
 *  @note For CDMA, this can be used to retrieve the SMS/EMS limits.
 *        length = max segment length.  size = max EMS length
 **/
typedef struct 
  {                          //        IN                                              OUT
  UInt16   size;             /**< unused */
  Boolean  substitution;     /**< Substitute characters if needed (GSM ONLY)                           */
  UInt16   length;           /**<                                              total length of message */
  UInt16   segmentCount;     /**<                                              number of segments used */
  Boolean  requestSmsInfo;   /**< unused */
}
PhnLibLengthInfoType;

/**
 *  SMS error codes obtained from the network in CDMA
 *  The first half of the enums are from IS-41D SMS cause codes
     ...
     of 0x00 to 0xFF. This does not have the cause codes from the range 0x80000
     upwards as the radio doesnt support them
 **/
typedef enum  {
  /* A. Network Problems:
  */
  SmsCauseAddressVacant                     = 0,
  ...
  SmsCauseOtherNetworkProblemMoreFirst   = 6,
  /* all values within this range are treated as
     OtherNetworkProblem
  */
  SmsCauseOtherNetworkProblemMoreLast    = 31,

  /* B. Terminal Problems:
  */
  SmsCauseNoPageResponse                   = 32,
  ...
  smsCauseOtherTerminalProblemMoreFirst  = 40,
  /* all values within this range are treated as
     OtherTerminalProblem
  */
  SmsCauseOtherTerminalProblemMoreLast   = 47,
  SmsCauseSmsDeliveryPostponedMoreFirst  = 48,
  SmsCauseSmsDeliveryPostponedMoreLast   = 63,

  /* C. Radio Interface Problems:
  */
  SmsCauseRadioIfResourceShortage         = 64,
  SmsCauseRadioIfIncompatible,
  SmsCauseOtherRadioIfProblem,
  SmsCauseOtherRadioIfProblemMoreFirst  = 67,
  /* all values within this range are treated as
     OtherRadioIfProblem
  */
  SmsCauseOtherRadioIfProblemMoreLast   = 95,

  /* D. General Problems:
  */
  SmsCauseUnexpected_Parm_Size               = 96,
  ...
  SmsCauseOtherGeneralProblemsMoreFirst  = 108,
  /* all values within this range are treated as
     OtherGeneralProblems
  */
  SmsCauseOtherGeneralProblemsMoreLast   = 255
}_SmsCauseCode;
 ...
 * MMS info structure.
 */
#define kMMSStringMaxLen                       128 /**< URL, MML, and user agent */
#define kMMSPixPlaceMaxLen                      20  /**< pix place address */
...
typedef struct _PhnLibMMSServerInfo
{
  UInt32  version;                                  /**< must be set to PhnLibMMSServerInfoVersion */
  UInt32  max_message_size;                         /**< max message size kB */
  UInt8   mms_server_name[kMMSStringMaxLen+1];      /**< MMS server [NULL terminated string] */
  UInt8   mml_server_name[kMMSStringMaxLen+1];      /**< MML server [NULL terminated string] */
  UInt8   pix_place_server[kMMSPixPlaceMaxLen+1];   /**< MMS Pix place address [NULL terminated string] */
  UInt8   user_agent[kMMSStringMaxLen+1];           /**< user agent string [NULL terminated string] */
  UInt8   maxRetries;                               /**< number of retry attempts */
  UInt8   timeout;                                  /**< MMSC timeout */
  Boolean deliveryAck;                              /**< delivery ack setting */
}
PhnLibMMSServerInfoType;
