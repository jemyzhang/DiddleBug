...
#define indicatorRemainingCallerIDChar   '~'  /**< CDMA Only -- Means the caller ID is the remaining caller (3+ calls scenario)  */
...
#define phnMaxUrlLen                128   /**< Max URL length */
...
typedef struct _PhnCDMAMediaTVSettings
{
  UInt32 version;                   /**< (1) should be set to the structure version that you are requesting */

  char   homePage[phnMaxUrlLen+1];  /**< NULL terminated string */
  UInt32 proxyAddr;                 /**< proxy IP address       */
  UInt16 proxyPort;                 /**< proxy port             */
  UInt32 streamBufferSize;          /**< stream buffer size     */
  UInt32 streamTimeout;             /**< streaming timeout      */
}
PhnCDMAMediaTVSettingsType;
