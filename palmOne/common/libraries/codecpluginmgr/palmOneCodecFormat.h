...
#define palmCodecVideoAVC			'AVCd'	/**< AVC(H.264) Video Format. */
...
typedef enum {
	...
	// AVC specific API
	palmCodecCtlAVCGetInfo,                 /**< Get the video frame width/ht info. */
	palmCodecCtlAVCGetDecodedFrame,         /**< Get the decoded frame. */
	palmCodecCtlAVCSetCropMode,             /**< Set the crop mode. */
	
	// Secure access to particular codec
	palmCodecCtlPutSeed,                 	/**< Give the codec a random number. */
	palmCodecCtlGetKey,         			/**< Get the encoded key from the codec. */
	palmCodecCtlPutKey,              		/**< Give the re-encoded key to the codec. */
	
	// AAC specific API
	palmCodecCtlAACGetInfo                  /**< Get the audio frame freq/chan info. */
...
/**
 * @brief AAC decoding parameters
 */
typedef struct {
	UInt32 fileFormat;				/**< RAW, ADTS, ADIF, MP4A and LATM */
	
	union {
		void *ascP;					/**< Parameter block for MP4A */
		void *esdsP;				/**< Parameter block for RAW */
	} u;
	
	void *frameP;					/**< First AAC frame */
	UInt32 frameSize;				/**< Frame size */
	UInt32 sampleRate;				/**> Sample rate returned */ 

} PalmAudioAACDecParamType;

typedef struct {
	UInt32 flags;      				/**< Set to VFS or FileStream */
...
/**
 * @brief AVC/H264 video format decoding parameters
 */
typedef struct {
    UInt8           *NALUnit;
    UInt32          NALUnitSize;
...
} PalmVideoAVCDecodeParamType;

/**
 * @brief AVC/H264 Custom Control API structure for GetInfo about frame 
 */
typedef struct {
    UInt16          frameWidth;          /**< width of lum. plane in pixels */
    UInt16          frameHeight;         /**< Ht of lum. plane in pixels */
    UInt16          frameWidthCropped;   /**< Width of cropped lum. plane in pixels */
    UInt16          frameHeightCropped;  /**< height of cropped lum. plane in pixels */
    UInt16          horizontalOffset;    /**< Horizontal offset of cropped lum. plane */
    UInt16          verticalOffset;      /**< Vertical offset of cropped lum. plane */
    UInt32          chromaFormat;        /**< Chroma format, only YUV420 for now */
} PalmVideoAVCCustomGetInfoType;

/**
 * @brief AVC/H264 Custom Control API structure for getting a decoded frame 
 */
typedef struct {
    PalmImagePlanarType  planarData;
    Int32                framesReady;
    UInt16               picWidth;            /**< width of lum. plane in pixels */
    UInt16               picWidthCropped;     /**< Width of cropped lum. plane in pixels */
    UInt16               picHeight;           /**< Ht of lum. plane in pixels */
    UInt16               picHeightCropped;    /**< height of cropped lum. plane in pixels */
    UInt16               stride;              /**< offset from start of one line of from to start of next */
    UInt32               compositionTimestamp;/**< timestamp moment in time decoded data to be played back */
    double               compositionTime;     /**< time describing moment in time decoded data to be played back */
} PalmVideoAVCCustomGetDecodedFrameType;

/**
 * @brief AVC/H264 Custom Control API structure for getting a decoded frame 
 */
typedef struct {
    Boolean              bCroppedMode;        /**< TRUE - output cropped size, FALSE - full size */      
} PalmVideoAVCCustomSetCropModeType;

/**
 * @brief AVC/H264 video format decoding parameters
 */
typedef struct {
    Int32                framesReady;         /**< No. of avail. decoded frames */
} PalmVideoAVCOutputBufferType;
