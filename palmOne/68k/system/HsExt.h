 ...
 *			     NULL terminator
 *  @retval 0 if no error, hsErrNotSupported if error occurs
 **/
Err
...
/**
 *  @brief Starts and stops memory tracing
 *
 *  @param tarce:	IN:  should tracing be started or stopped
 *  if true tracing will be started.
 * 
 *  called from the 68K controling (test) app.
 **/
void 
HsStartMemoryTracing(Boolean trace)
				SYS_SEL_TRAP (sysTrapHsSelector, hsSelStartMemoryTracing);
