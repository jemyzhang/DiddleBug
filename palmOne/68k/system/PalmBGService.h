 ...
 ******************************************************************************/
/** 
 * @defgroup	BGService Background Service Library
 * @brief		This library provides support for applications to perform background
 *				network request bypassing NetLib through the default connection.
 *
 * Application can use this library to connect to a server on a network and perform
 * network request by sending protocol-specific string and receiving the returned response.
 * Applications can also utilize this library to make background HTTP request by connecting
 * to a web server at port 80 and sending HTTP GET/POST string with specific URL.
 *
 * Network requests are queued up and the responses are returned through the callback function
 * that is passed to the network request API.
 *
 * @{
 * @}
 */
 ...
 * @version 1.0
 * @brief	Public 68K header file for palmOne Background Network Service Library
 *
 * This file contains the function prototypes and structures for the Background Service which
 ...
 *
 * @return IBG_SUCCESS on success, otherwise IBG_ERRRQSTDONE if request
 ...
 * @return One of the IBG_xxx return codes defined above.  Currently any return
 *         value is ignored.
 */
 ...
 * 
 * @return IBG_SUCCESS if accepted, otherwise another error code defined above
 */
