...
#define kSndFileStreamLibVersion	sysMakeROMVersion(1, 4, 0, sysROMStageRelease, 20)	/**<Version of the sound file stream library. */
/*@}*/
 ...
 * 
 * In playback mode, it means that the file has completely played. 
 * In recording mode, it means that it reaches the recording limit. 
 */
/*@{*/
...
/**
 * Notification broadcast by SndFileStream when it exceeds the recording limit.
 * Currently this notification is handled by the libary itself, for example,
 * stop the recording stream. The application should NOT expect to recieve this notification
 * even it register it. Instead it should expect sndFileStreamNotifyStreamComplete notification to 
...
#define kSndFileTypeAAC 'AACf'	/**<AAC file   -> kSndFileFormatAAC      */
/*@}*/

/**
 * @brief SndFileStreamDeviceControl param when cmd == hsSoundLibCreatorID
 *
 * Contains hsFormat/hsCmd pair to be used with PmSndStream() calls.
 */
typedef struct tagSndFileStreamHsSoundCommandParam
{
	UInt16	hsFormat;
	UInt16	hsCmd;
}
SndFileStreamHsSoundCommandParam, *SndFileStreamHsSoundCommandParamPtr;
