...
#define PmSndStreamPause(libRef,stream)  HsSndFormatPlay( libRef, (HsSndFormatType) pmSndFormatPauseStream,  (HsSndCmdType) 0, (void*) 0, (UInt32) stream )
#define PmSndStreamResume(libRef,stream) HsSndFormatPlay( libRef, (HsSndFormatType) pmSndFormatResumeStream, (HsSndCmdType) 0, (void*) 0, (UInt32) stream )
#define PmSndStreamStoppable(libRef,stream) HsSndFormatPlay( libRef, (HsSndFormatType) pmSndFormatSetStoppable,  (HsSndCmdType) 0, (void*) 0, (UInt32) stream )
/*@}*/
...
///
/// @param libRef:  IN: Library reference number (from SysLibLoad).
/// @param clkRef:  IN: Clock reference number (see HsSoundLibCommon.h).
/// @param ticksPerSecondP  OUT: Returns the tick per second
/// @return Library error code.
Err    PmSndClkGetTicksPerSecond ( UInt16 libRef, PmClkTickRef clkRef, UInt32* ticksPerSecondP )
          SYS_TRAP (kPmSoundLibTrapClkGetTicksPerSecond);

/// Get current clock tick count.
///
/// @param libRef:  IN: Library reference number (from SysLibLoad).
/// @param clkRef:  IN: Clock reference number (see HsSoundLibCommon.h).
/// @param clockTicksP  OUT: Returns the current clock tick count.
/// @return Library error code.
Err    PmSndClkGetTickCount ( UInt16 libRef, PmClkTickRef clkRef, PmSndU64* clockTicksP )
          SYS_TRAP (kPmSoundLibTrapClkGetTickCount);

/// Get current stream sample position and current clock tick count.
///
/// @param libRef:  IN: Library reference number (from SysLibLoad).
/// @param streamRef IN: Sound stream channel reference id.
/// @param sampleNumberP: OUT: Returns the current sample number for the stream.
/// @param clkRef:  IN: Clock reference number (see HsSoundLibCommon.h).
/// @param clockTicksP  OUT: Returns the current clock tick count.
/// @return Library error code.
Err    PmSndGetPosition ( UInt16 libRef, SndStreamRef streamRef, PmSndU64* sampleNumberP, PmClkTickRef clkRef, PmSndU64* clockTicksP )
          SYS_TRAP (kPmSoundLibTrapGetPosition);
