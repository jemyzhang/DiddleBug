...
/// Minimum db version of TonesLibrary database that implements escalation
/// Done this way since we have no GetLibVersion() API.
...
/// @param name:		I/O: Pointer to the returned tone name.
/// @param maxLength:	IN:  Size of the name buffer in bytes.
/// @return Library error code.
...
///
/// @param refNum:	IN:	Library reference number from SysLibLoad.
/// @param set:		IN: true if setting, false if querying.
/// @param value:   IN: Value to set if setting, queried value if querying.
/// @return Library error code.
Err TonesLibEscalateRingTones (UInt16 refNum, Boolean set, Boolean *value)
                 SYS_TRAP (tonesLibTrapEscalateRingTones);

/// Play a ring tone given its ID.  The difference between this and PlayTone is
/// that tones played this way are affected by TonesLibEscalateRingTones().
///
/// @param refNum:		IN:	Library reference number from SysLibLoad.
/// @param toneId:		IN:	ID of the tone to play.
/// @param playCount:	IN: Number of times you want the tone repeated.
...
///							or play in the background and return immediately.
/// @return Library error code.
Err TonesLibPlayRingTone (UInt16 refNum, ToneIdentifier toneId, UInt16 playCount,
	ToneVolume volume, ToneVibrateType vibrate, Boolean blocking)
                 SYS_TRAP (tonesLibTrapPlayRingTone);

// Save a copy of undeletable tones into a separate hidden db
// so that we can copy them back to main MIDI db. This is in case that delete
// protected tones got lost during recovery hot sync where MIDI db from the 
// computer profile was copied over to the device.
// Called from SoundPhone Panel at SyncStart notification
// 
// @param   refNum  IN:	Library reference number from SysLibLoad.        
// @return  Library error code
Err TonesLibSaveUndeletableTones(UInt16 refNum)
                 SYS_TRAP (tonesLibTrapSaveUndeletableTones);

// After hot Sync has finished copy back delete protected tones from 
// backup db to main MIDI db. See TonesLibSaveUndeletableTones also.  
// Called from SoundPhone Panel at SyncFinish notification
// 
// @param   refNum  IN:	Library reference number from SysLibLoad.        
// @return  Library error code
Err TonesLibCopyBackUndeletableTones(UInt16 refNum)
                 SYS_TRAP (tonesLibTrapCopyBackUndeletableTones);
